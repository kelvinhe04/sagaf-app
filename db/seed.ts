// db/seed.ts — Datos de demo conforme al documento académico (Parcial ISA 4 V2.0)
...
linkSOPL.run('so_banco_demo', 'pl_bank_natural');
linkSOPL.run('so_banco_demo', 'pl_bank_legal');
linkSOPL.run('so_inmob_demo', 'pl_realestate');
...
const bankNatural = [
  ...
  'Constancia de Ingresos (Declaraciones, Ficha o Talonario de Cheque)',
  'Carta de Trabajo',
  'Tarjeta de Firmas',
  'Historial de Crédito',
  'Referencias Bancarias',
  'Referencias Comerciales y/o Profesionales',
  'Comunicaciones internas y externas para descartar hechos inusuales',
...
const bankLegal = [
  ...
  'Pacto social o documento constitutivo, poderes, etc.',
  'Aviso de Operación o documento equivalente a Licencia Comercial',
  'Historial de Crédito',
  'Declaraciones de Rentas obtenidas desde la apertura de la relación',
  'Referencias Bancarias',
  'Referencias Comerciales y/o Profesionales',
  'Dignatarios y Directores de la Persona Jurídica y actualizaciones',
  'Estados Financieros de la persona jurídica',
  'Información sobre titular de acciones emitidas y custodios',
  'Evidencia de acciones emitidas, tenedores y/o custodios',
  'Información obtenida sobre Beneficiarios Finales',
  'Identificación de Beneficiarios Finales',
  'Comunicaciones de descarte de la inusualidad',
  'Estado de cuenta de los 2 últimos años en PDF y Excel',
  '80% de los Créditos recibidos durante el último año',
  '80% de los Débitos realizados durante el último año',
  'Copia de cheques con anversos y reversos',
  'Copia completa de transferencias internacionales (mensaje Swift)',
  'Datos de ACH enviados y/o recibidos',
...
// =====================================================================
// 4. Usuarios demo — todos con password "password123" (bcrypt)
// =====================================================================
...
const usuariosDemo: Array<[string, string, string, string, string | null]> = [
  ['u_so_banco',     'Banco Nacional Demo · Cumplimiento',     'cumplimiento@bancodemo.com',        'rol_sujeto_obligado', 'so_banco_demo'],
  ['u_so_inmob',     'Inmobiliaria Istmo Demo · Cumplimiento', 'cumplimiento@inmobiliariademo.com', 'rol_sujeto_obligado', 'so_inmob_demo'],
...
// =====================================================================
// 6. ROS de demo (3 reportes, alineados con la bandeja del Prototipo.html)
// =====================================================================
...
insertROS.run(
  'ros_001', 'ROS-2026-000248', 'so_banco_demo', 'pl_bank_legal',
  'Lic. Roberto Mendoza', 'cumplimiento@bancodemo.com',
...
insertROS.run(
  'ros_002', 'ROS-2026-000249', 'so_inmob_demo', 'pl_realestate',
  'Lic. Patricia Vásquez', 'cumplimiento@inmobiliariademo.com',
...
insertROS.run(
  'ros_003', 'ROS-2026-000250', 'so_banco_demo', 'pl_bank_natural',
  'Lic. Roberto Mendoza', 'cumplimiento@bancodemo.com',
...
// Vinculación detectada entre ROS-001 y ROS-003 (Carlos Alberto Pérez es ordenante de ros_003 y beneficiario de ros_001)
db.prepare(`
  INSERT INTO vinculo_intersectorial (id, ros_origen_id, ros_destino_id, tipo_vinculo, descripcion, confirmado, fecha_deteccion)
  VALUES (?, ?, ?, 'persona', 'Coincidencia: Carlos Alberto Pérez (cédula 8-482-917) figura como beneficiario en ROS-2026-000248 y ordenante en ROS-2026-000250.', 0, CURRENT_TIMESTAMP)
`).run(uid(), 'ros_001', 'ros_003');

// =====================================================================
// 7. Evento de auditoría inicial — semilla del log
// =====================================================================
db.prepare(`
  INSERT INTO evento_auditoria
    (id, usuario_id, usuario_correo, rol, modulo, accion, resultado, recurso_afectado, detalle, criticidad)
  VALUES (?, NULL, 'system', 'system', 'system', 'seed_inicial', 'exito', NULL, 'Datos demo cargados al iniciar el sistema', 'normal')
`).run(uid());
...
console.log(`  • ${usuariosDemo.length} usuarios demo`);
console.log(`  • ${personasMock.length} personas mock`);
console.log(`  • 3 ROS demo + 1 vínculo intersectorial`);
